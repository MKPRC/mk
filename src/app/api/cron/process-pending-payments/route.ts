  ...
  try {
    ...
    for (const subscription of pendingSubscriptions) {
      try {
        ...
        const orderName = `${subscription.membership_type} 멤버십 - 첫 결제`;
        ...
        if (!paymentResponse.ok) {
          ...
          console.error(`첫 결제 실패 (${subscription.id}):`, errorData);
          
          // V2 에러 코드 처리
          let failureReason = errorData.message || '첫 결제 실패';
        ...
        console.log(`첫 결제 성공 (${subscription.id}):`, paymentData);
    ...
    console.error('대기 결제 처리 오류:', error);
